...
import { FileInput } from '../../ai-files/interfaces/file-input.interface';
import { FileType } from '../../ai-files/enums/file-type.enum';
...
export class QuestionValidationService {
  ...
  /**
   * Helper to parse AI service response (can be string or ContentBlock array)
   */
  private parseAIResponse(response: any): any {
    if (typeof response === 'string') {
      return JSON.parse(response);
    }
    // If it's an array of ContentBlocks, extract text and parse
    if (Array.isArray(response)) {
      const textContent = response
        .map((block: any) => (block.text ? block.text : JSON.stringify(block)))
        .join('');
  ...
  /**
   * Main validation method that routes to specific validators based on question type
   */
  async validateAnswer(
    ...
    switch (question.type) {
      // AUTO validations (simple comparison)
  ...
  private async validateMultipleChoice(question: any, userAnswer: string): Promise<ValidationResult> {
  ...
  private async validateUnscramble(question: any, userAnswer: string): Promise<ValidationResult> {
  ...
  private async validateTenses(question: any, userAnswer: string): Promise<ValidationResult> {
    const correctAnswer = question.answer;
    const isCorrect = userAnswer === correctAnswer;
  ...
  private async validateReadIt(question: any, userAnswer: any): Promise<ValidationResult> {
    // userAnswer should be an object with sub-question IDs as keys
    let totalPoints = 0;
    let earnedPoints = 0;
  ...
  private async validateWordMatch(question: any, userAnswer: string): Promise<ValidationResult> {
    const correctAnswer = question.answer;
    const isCorrect = userAnswer === correctAnswer;
  ...
  private async validateTopicBasedAudio(question: any, userAnswer: any): Promise<ValidationResult> {
    // Similar to read_it - validate sub-questions
    let totalPoints = 0;
    let earnedPoints = 0;
  ...
  private async validateLyricsTraining(question: any, userAnswer: string): Promise<ValidationResult> {
    const correctAnswer = question.answer;
    const isCorrect = userAnswer === correctAnswer;
  ...
  private async validateFastTest(question: any, userAnswer: string): Promise<ValidationResult> {
    const correctAnswer = question.answer;
    const isCorrect = userAnswer === correctAnswer;
  ...
  /**
   * Helper: Check if a word can be formed from the grid letters
   */
  private canFormWordFromGrid(word: string, grid: string[][]): boolean {
    if (!grid || !Array.isArray(grid) || grid.length === 0) {
      return false;
    }

    // Flatten grid to get all available letters (case-insensitive)
    const availableLetters = grid
      .flat()
      .map((letter) => letter.toLowerCase());

    // Count letter frequency in available letters
    const availableFrequency: Record<string, number> = {};
    for (const letter of availableLetters) {
      availableFrequency[letter] = (availableFrequency[letter] || 0) + 1;
    }

    // Count letter frequency needed for the word
    ...
    // Check if all letters in word are available in sufficient quantity
  ...
  private async validateWordbox(question: any, userAnswer: string[]): Promise<ValidationResult> {
    // Get the grid from question content
    const grid = question.content as string[][];

    if (!grid || !Array.isArray(grid)) {
      throw new BadRequestException('Invalid wordbox grid structure');
    }

    // Get maxWords from question configurations
    const maxWordsConfig = question.configurations?.find(
      (config: any) => config.metaKey === 'maxWords'
    );
    const maxWords = maxWordsConfig ? parseInt(maxWordsConfig.metaValue) : 5;

    // VALIDATION 1: Check if words can be formed from grid letters
    const wordsNotInGrid: string[] = [];
    ...
    // If no words can be formed from grid, return error with 0 points
    ...
    // VALIDATION 2: Validate that formed words are valid English words using AI
    // Only validate words that can be formed from the grid
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "Formaste X palabras válidas."
    ...
    try {
      ...
      // Calculate points based on: (validWords / maxWords) * questionPoints
      const pointsEarned = Math.round((validWords.length / maxWords) * question.points);

      // Consider correct if all submitted words that could be formed are valid English words
      ...
      throw new BadRequestException(
        `Error validating wordbox with AI: ${error.message || 'Unknown error'}`
      );
    }
  }
  ...
  private async validateSpelling(question: any, audioFile: any): Promise<ValidationResult> {
    if (!audioFile) {
      throw new BadRequestException('Audio file is required for spelling validation');
    }
    ...
    try {
      ...
      if (!result.success) {
        throw new BadRequestException('Error validating spelling');
      }
      ...
      throw new BadRequestException('Error validating spelling with AI');
    }
  }
...
Return a JSON object with this structure:
{
  ...
  private async validateReportIt(question: any, userAnswer: string): Promise<ValidationResult> {
    const originalSentence = question.content as string;
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "¡Excelente! Tu discurso indirecto es gramaticalmente correcto."
    ...
    try {
      ...
      throw new BadRequestException('Error validating reported speech with AI');
    }
  }

  private async validateGossip(question: any, audioFile: any): Promise<ValidationResult> {
    if (!audioFile) {
      throw new BadRequestException('Audio file is required for gossip validation');
    }
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "¡Excelente pronunciación y precisión!"
    ...
    try {
      ...
      if (!result.success) {
        throw new BadRequestException('Error processing audio');
      }

      const aiResult = JSON.parse(result.data as string);
      ...
      throw new BadRequestException('Error validating gossip with AI');
    }
  }

  private async validateSentenceMaker(question: any, userAnswer: string): Promise<ValidationResult> {
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "¡Buena oración! Se necesitan pequeñas mejoras gramaticales."
    ...
    try {
      ...
      throw new BadRequestException('Error validating sentence with AI');
    }
  }

  private async validateTales(question: any, userAnswer: string): Promise<ValidationResult> {
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "¡Excelente historia! Muy creativa con buena gramática."
    ...
    try {
      ...
      throw new BadRequestException('Error validating tale with AI');
    }
  }

  private async validateSuperbrain(question: any, audioFile: any): Promise<ValidationResult> {
    if (!audioFile) {
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "¡Buena pronunciación y contenido relevante!"
    ...
    try {
      ...
      throw new BadRequestException('Error validating superbrain with AI');
    }
  }

  private async validateTellMeAboutIt(question: any, audioFile: any): Promise<ValidationResult> {
    if (!audioFile) {
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "¡Historia interesante con buena pronunciación!"
    ...
    try {
      ...
      throw new BadRequestException('Error validating story with AI');
    }
  }

  private async validateDebate(question: any, audioFile: any): Promise<ValidationResult> {
    if (!audioFile) {
...
Return a JSON object with this structure:
{
  ...
  "feedbackSpanish": "¡Argumentos sólidos con buena evidencia de apoyo!"
    ...
    try {
      ...
      throw new BadRequestException('Error validating debate with AI');
    }
  }
}
