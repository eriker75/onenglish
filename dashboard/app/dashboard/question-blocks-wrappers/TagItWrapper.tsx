...
export default function TagItWrapper() {
  ...
  const createQuestionMutation = useMutation({
    mutationFn: async (formData: FormData) => {
      // Note: Controller does not explicitly have @FormDataRequest but DTO has @IsFile. 
      // We assume backend handles FormData or we need to check if it fails.
  ...
  return (
    <div className="space-y-6 p-6 bg-white rounded-xl shadow-sm border border-gray-200">
      <div className="flex justify-between items-center border-b pb-4">
        <h2 className="text-xl font-bold text-gray-800">Create Tag It Question</h2>
        <Button
          onClick={handleSave}
