...
interface FastTestProps {
  question?: string;
  instructions?: string;
  content?: string[]; // The sentence split by the blank
  options?: string[];
...
export default function FastTest({
  ...
  // Join content with a placeholder for editing
  const [contentText, setContentText] = useState(
    content.length > 0 ? content.join(" ___ ") : ""
  );
  ...
  const handleContentTextChange = (value: string) => {
    ...
    // Split by "___" to get content array
    // We split by exactly 3 underscores and trim whitespace
    const splitContent = value.split("___").map(s => s.trim()).filter(s => s !== "");
    onContentChange?.(splitContent);
  };
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {/* Row 2: Sentence/Correct Answer and Options (Two Columns) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Left Column: Sentence and Correct Answer */}
        <div className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Enter the sentence with a blank (use ___)"
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#44b07f] focus:border-transparent"
            />
            <p className="text-xs text-gray-500 mt-1">
              Use "___" (three underscores) to indicate where the word should be selected.
            </p>
          </div>
