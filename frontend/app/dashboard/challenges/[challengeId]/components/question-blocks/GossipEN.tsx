...
export default function GossipEN({
  question = "",
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Correct Transcription (English) *
        </label>
        <textarea
          value={correctTranscriptionText}
          onChange={(e) => handleCorrectTranscriptionChange(e.target.value)}
          placeholder="Enter the correct English transcription..."
          rows={4}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#44b07f] focus:border-transparent"
        />
        <p className="text-xs text-blue-600 mt-1">
          ℹ️ This serves as the base answer. If student's answer doesn't match exactly, AI will review it as a fallback.
        </p>
      </div>
    </div>
  );
